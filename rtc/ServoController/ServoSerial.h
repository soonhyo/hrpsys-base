...
public:
  ...
  int setID(int id, unsigned char new_id) {// #4
    ...
    if (sendPacket(0xFAAF, id, 0x00, 0x04, 1, 1, &new_id) < 0) return -1;
    if (sendPacket(0xFAAF, new_id, 0x40, 0xFF, 0, 0, NULL) < 0) return -1;  // Write to Flash ROM
  ...
  int setReverse(int id, int is_reverse) {// #5
    ...
    data[0] = is_reverse;
    if (sendPacket(0xFAAF, id, 0x00, 0x05, 1, 1, data) < 0) return -1;
    if (sendPacket(0xFAAF, id, 0x40, 0xFF, 0, 0, NULL) < 0) return -1;  // Write to Flash ROM
    // I don't know why, but after the command above, powering off servo is required to get return packet from servo
    // setReset instead of powering off doesn't work
    return 0;
  }
